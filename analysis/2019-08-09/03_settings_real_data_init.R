...
num_counts <- data_current$Sum
D  <- ncol(y_t)
reg_scale <- 1e2
state_scale <- rep(1e4, times = D)
za1_t <- cbind(rep(1, times = TT),
               data_current$cleid/reg_scale,
               data_current$dfeid/reg_scale,
               data_current$estcd/reg_scale,
               data_current$ngeid/reg_scale)
za2_t <- za1_t
za3_t <- za1_t
za4_t <- za1_t
za5_t <- za1_t
