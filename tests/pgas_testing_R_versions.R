pgas2 <- function(MM, TT, N, num_counts = num_counts,
  ...
  for (m in 2:2) {
    ...
    mu_xa1 <- round(mu_xa1, digits = 3)
    Omega_xa1 <- round(Omega_xa1, digits = 3)
    # phi_xa1[m]   <- mu_xa1[1]
    # bet_xa1[, m] <- mu_xa1[-1]
    ...
    mu_xa2 <- round(mu_xa2, digits = 3)
    Omega_xa2 <- round(Omega_xa2, digits = 3)
    # phi_xa2[m]   <- mu_xa2[1]
    # bet_xa2[, m] <- mu_xa2[-1]
    ...
    mu_xa3       <- round(mu_xa3, digits = 3)
    Omega_xa3    <- round(Omega_xa3, digits = 3)
    # phi_xa3[m]   <- mu_xa3[1]
    # bet_xa3[, m] <- mu_xa3[-1]
    ...
    mu_xa4 <- round(mu_xa4, digits = 3)
    Omega_xa4 <- round(Omega_xa4, digits = 3)
    # phi_xa4[m]   <- mu_xa4[1]
    # bet_xa4[, m] <- mu_xa4[-1]
    ...
    mu_xa5       <- round(mu_xa5, digits = 3)
    Omega_xa5    <- round(Omega_xa5, digits = 3)
    # phi_xa5[m]   <- mu_xa5[1]
    # bet_xa5[, m] <- mu_xa5[-1]
    ...
    mu_xa6       <- round(mu_xa6, digits = 3)
    Omega_xa6    <- round(Omega_xa6, digits = 3)
    # phi_xa6[m]   <- mu_xa6[1]
    # bet_xa6[, m] <- mu_xa6[-1]
  ...
  # return(list(sig_sq_xa1[1:4], Omega_xa1, mu_xa1,
  #             sig_sq_xa2[1:4], Omega_xa2, mu_xa2,
  #             sig_sq_xa3[1:4], Omega_xa3, mu_xa3,
  #             sig_sq_xa4[1:4], Omega_xa4, mu_xa4,
  #             sig_sq_xa5[1:4], Omega_xa5, mu_xa5,
  #             sig_sq_xa6[1:4], Omega_xa6, mu_xa6))
  m1 <- 1
  m2 <- 2
  return(list(Xa1[m1, ], Xa2[m1, ],
              Xa3[m1, ], Xa4[m1, ],
              Xa5[m1, ], Xa6[m1, ],
              # sig_sq_xa1[1:4],
              # sig_sq_xa2[1:4],
              ...
              # Omega_xa6, mu_xa6
              #
              phi_xa1[m2], bet_xa1[, m2],
              phi_xa2[m2], bet_xa2[, m2],
              phi_xa3[m2], bet_xa3[, m2],
              phi_xa4[m2], bet_xa4[, m2],
              phi_xa5[m2], bet_xa5[, m2],
              phi_xa6[m2], bet_xa6[, m2]
              ))
  # return(list(Xa1[m - 1, 2:TT], Xa2[m - 1, 2:TT],
  #             Za1[2:TT, , drop = F], Za2[2:TT, , drop = F],
  #             phi_xa2[m - 1], bet_xa2[, m - 1]))
  ...
  # return(list(sig_sq_xa1[1:4], Omega_xa1, mu_xa1))
  # return(list(regs_a1, x_lhs, crossprod(regs_a1, x_lhs)))
