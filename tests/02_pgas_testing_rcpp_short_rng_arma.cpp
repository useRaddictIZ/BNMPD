...
// [[Rcpp::export]]
mat cbpf_as_c5_full(const int& N,
                    const int& TT,
                    const int& DD,
  ...
  // adding constants (state process mean) for part particle propagation
  // ancestors
  umat a(N, TT);
  uvec id_as = linspace<uvec>(0L, N - 1L, N);
...
List pgas2_short_rng_arma(const int& N,
  ...
  //// I. If starting values are one (consant) value for all t=1,...,TT per state d
  if (check_state_init_type == DD) {
    for (int d = 1; d < DD+1; ++d) {
      (Xa.submat(TT*(d -1), 0, TT*d - 1, 0)).fill(traj_init(d - 1));
    }
  }
  //// II. If starting values are is a full trajectory of length TT per state d
  if (check_state_init_type == DD*TT) {
    Xa.col(0) = traj_init;
  }
  // II. run cBPF and use output as first conditioning trajectory
  for (int d = 0; d < DD; ++d) {
    Za_beta.col(d) = Z.submat(0, id_bet(d), TT - 1, id_bet(d + 1) - 1) * bet.submat(id_bet(d), 0, id_bet(d + 1) - 1, 0);
  }
  out_cPF = cbpf_as_c5_full(N, TT, DD,
                            num_counts, y,
                            Za_beta,
                            sig_sq_x.col(0),
                            phi_x.col(0),
  ...
  for (int m = 1; m < MM; ++m) {
    ...
    for(int d = 0; d < DD; ++d) {
      ...
      Za_beta.col(d) = Z.submat(0, id_bet(d), TT - 1, id_bet(d + 1) - 1) * bet.submat(id_bet(d), m, id_bet(d + 1) - 1, m);
    }
    out_cPF = cbpf_as_c5_full(N, TT, DD,
                              num_counts, y,
                              Za_beta,
