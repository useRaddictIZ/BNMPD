...
List pgas2_full(const int& N,
  ...
  // Xa1.row(0) = out_cPF.col(0).t();
  // Xa2.row(0) = out_cPF.col(1).t();
  // Xa3.row(0) = out_cPF.col(2).t();
  // Xa4.row(0) = out_cPF.col(3).t();
  // Xa5.row(0) = out_cPF.col(4).t();
  // Xa6.row(0) = out_cPF.col(5).t();
  ...
  for (int m = 1; m < 2; ++m) {
    ...
    // Z.col(id_zet(0)+1) = (Xa1.submat(m-1, 0, m-1, TT - 2)).t();
    // Z.col(id_zet(1)+1) = (Xa2.submat(m-1, 0, m-1, TT - 2)).t();
    // Z.col(id_zet(2)+1) = (Xa3.submat(m-1, 0, m-1, TT - 2)).t();
    // Z.col(id_zet(3)+1) = (Xa4.submat(m-1, 0, m-1, TT - 2)).t();
    // Z.col(id_zet(4)+1) = (Xa5.submat(m-1, 0, m-1, TT - 2)).t();
    // Z.col(id_zet(5)+1) = (Xa6.submat(m-1, 0, m-1, TT - 2)).t();
    // 1. pars for xa1_t process --------------------------------------------
    // temp_vec_col = (Xa1.submat(m - 1, 1, m - 1, TT - 1)).t();
    // z_add =  Za1.submat(1, 0, TT - 1, dim_pars(0) - 3) * bet.submat(id_bet(0),  m - 1, id_bet(0 + 1) - 1,  m - 1);
    // (Xa1.submat(m - 1, 0, m - 1, TT - 2)).t()
    // mu_xa(0, 0) = mu_xa1;
    // Omega_xa(0) = Omega_xa1;
    ...
    // temp_vec_col = Xa.submat( 1, m - 1, TT - 1, m - 1);
    // z_add =  Z.submat(0, id_zet(0) + 1, TT - 2, id_zet(1) - 1) * bet.submat(id_bet(0),  m - 1, id_bet(0 + 1) - 1,  m - 1);
    // temp_vec_col2 = temp_vec_col - f_cpp_vech(Xa.submat(0, m - 1, TT - 2, m - 1),
    //                                           phi_x(0, m - 1),
    //                                           z_add);
    // err_siq_sq_x = (dot(temp_vec_col2, temp_vec_col2)) * 0.5;
    // sig_sq_x(0, m)  = 1/(R::rgamma(prior_a, 1.0/(prior_b + err_siq_sq_x)));
    // Omega_xa1    = inv((trans(Z.cols(id_zet(0), id_zet(1) - 1)) * Z.cols(id_zet(0), id_zet(1) - 1))/sig_sq_x(0, m) + prior_V_xa1);
    // Omega_xa(0) = Omega_xa1;
    // mu_xa1       = Omega_xa1 * (trans(Z.cols(id_zet(0), id_zet(1) - 1)) * temp_vec_col)/sig_sq_x(0, m);
    // mu_xa(0) = mu_xa1;
    //
    //
    //
    //
    // double digits = 1000;
    // sig_sq_x(0, m) = round(sig_sq_x(0, m)*digits)/digits;
    // sig_sq_x(1, m) = round(sig_sq_x(1, m)*digits)/digits;
    // sig_sq_x(2, m) = round(sig_sq_x(2, m)*digits)/digits;
    // sig_sq_x(3, m) = round(sig_sq_x(3, m)*digits)/digits;
    // sig_sq_x(4, m) = round(sig_sq_x(4, m)*digits)/digits;
    // sig_sq_x(5, m) = round(sig_sq_x(5, m)*digits)/digits;
    // phi_x(0, m) = round(phi_x(0, m)*digits)/digits;
    // phi_x(1, m) = round(phi_x(1, m)*digits)/digits;
    // phi_x(2, m) = round(phi_x(2, m)*digits)/digits;
    // phi_x(3, m) = round(phi_x(3, m)*digits)/digits;
    // phi_x(4, m) = round(phi_x(4, m)*digits)/digits;
    // phi_x(5, m) = round(phi_x(5, m)*digits)/digits;
    // bet.submat(id_bet(0), m, id_bet(0 + 1) - 1, m).transform([](double val){return(round(val*1000)/1000);});
    // bet.submat(id_bet(1), m, id_bet(1 + 1) - 1, m).transform([](double val){return(round(val*1000)/1000);});
    // bet.submat(id_bet(2), m, id_bet(2 + 1) - 1, m).transform([](double val){return(round(val*1000)/1000);});
    // bet.submat(id_bet(3), m, id_bet(3 + 1) - 1, m).transform([](double val){return(round(val*1000)/1000);});
    // bet.submat(id_bet(4), m, id_bet(4 + 1) - 1, m).transform([](double val){return(round(val*1000)/1000);});
    // bet.submat(id_bet(5), m, id_bet(5 + 1) - 1, m).transform([](double val){return(round(val*1000)/1000);});
    out_cPF = cbpf_as_c2_full(N, TT,
                              num_counts, y,
                              Za1, Za2, Za3, Za4, Za5, Za6,
                              sig_sq_x(0, m),
    ...
    // (Xa.submat(0, m -1, TT - 1, m -1)).transform([](double val){return(round(val*1000)/1000);});
    // (Xa.submat(TT, m -1, TT*2 - 1, m -1)).transform([](double val){return(round(val*1000)/1000);});
    // (Xa.submat(TT*2, m -1, TT*3 - 1, m -1)).transform([](double val){return(round(val*1000)/1000);});
    // (Xa.submat(TT*3, m -1, TT*4 - 1, m -1)).transform([](double val){return(round(val*1000)/1000);});
    // (Xa.submat(TT*4, m -1, TT*5 - 1, m -1)).transform([](double val){return(round(val*1000)/1000);});
    // (Xa.submat(TT*5, m -1, TT*6 - 1, m -1)).transform([](double val){return(round(val*1000)/1000);});
  }
  List all_traj(D);
  for (int d = 0; d<D; ++d){
    all_traj(d) = Xa.submat(TT*d, 0, TT*(d + 1) - 1, MM - 1);
  }
  return(List::create(Rcpp::Named("sigma_sq_xa1") = sig_sq_x.row(0),
  ...
  //                     id_bet, dim_pars(0), id_bet(1),Xa1.row(0),
  //                     Xa2.row(0), Xa3.row(0), Xa4.row(0), Xa5.row(0), Xa6.row(0)));
  // double m = 0;
  // return(List::create(Xa.submat(0, m, TT - 1, m).t(),
  //                     Xa.submat(TT, m, TT*2 - 1, m).t(),
  //                     Xa.submat(TT*2, m, TT*3 - 1, m).t(),
  //                     Xa.submat(TT*3, m, TT*4 - 1, m).t(),
  //                     Xa.submat(TT*4, m, TT*5 - 1, m).t(),
  //                     Xa.submat(TT*5, m, TT*6 - 1, m).t()));
  // double m =0;
